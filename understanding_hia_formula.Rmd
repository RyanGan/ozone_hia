...
Going to start with a simulated 'true' association for a binary outcome. I'm going to use Texas as an example. The population of Texas as of July 1st is 27,862,596. The estimated number of Children (under 18) in the state is 26.2%, so 7,300,000 children under the age of 18. 

Based on CDC estimates of children under 15 that had reported asthma in the past year that I think we estimated by scrapping and then using a meta-analysis is around 548,901. There is some disconnect between year and age, but this suggests the prevalence of childhood ashtma in Texas is around 7.5%. I think that sounds reasonable. 

I'm going to also inflate the exposure, lets say smoke day to 10%. It's probably more like <5%.

```{r population simulation, message=F}
# simulation population data ----
set.seed(987)
pop_data <- data_frame(x = rep(seq(from = 1, to = 1000, by =1), 100)) %>% 
  arrange(x) %>%  # sort x by ascending values
  cbind(y = rep(seq(from = 1, to = 1000, by =1), 100)) %>% 
  # create randomly assigned proportion of disease
  mutate(exposure = rbinom(100000, size = 1, prob = 0.1),
...
logit_form = -2.5 + 0.09*pop_data$exposure # linear combination with a bias
...
# simulate disease
pop_data <- pop_data %>% 
  cbind(disease = rbinom(100000, size = 1, prob = pr)) %>% 
...
In the simulation above, I've created a binary exposure where roughly 10% of the population had been exposed to a smoke day. I then defined the exposure/response relationship to be an odds ratio of ~1.09, or a ~9% increase in risk on a smoke day (or a beta of 0.09). Also, the baseline proportion of disease in this population is ~0.076 or 7.6%. Roughly the prevalence of asthma in Texas. Below are our estimates of 'truth'.
